    ...
    <section>
        <div class="bg-primary-white rounded-lg shadow-lg max-w-4xl">
            ...
            <form @submit.prevent="onSubmit" class="p-8 space-y-6">
                ...
                <!-- Title -->
                <TheField id="title" v-model="formData.title" label="Title" placeholder="Enter title"></TheField>
                <div v-if="errorMessage.title" class="text-sm text-red-500">{{ errorMessage.title }}</div>

                <!-- Description -->
                <TheField id="description" v-model="formData.description" label="Description"
                    placeholder="Enter description"></TheField>
                <div v-if="errorMessage.description" class="text-sm text-red-500">{{ errorMessage.description }}</div>

                <!-- Price -->
                <TheField id="price" v-model="formData.price" label="Price" placeholder="Enter price"></TheField>
                <div v-if="errorMessage.price" class="text-sm text-red-500">{{ errorMessage.price }}</div>

                <!-- Pet Type -->
                <TheField id="pet_type" v-model="formData.pet_type" label="Pet Type" placeholder="Select pet type">
                    <select>
                        <option value="cat">Cat</option>
                        <option value="dog">Dog</option>
                    </select>
                </TheField>
                <div v-if="errorMessage.pet_type" class="text-sm text-red-500">{{ errorMessage.pet_type }}</div>

                <!-- Available Amount -->
                <TheField id="available_amount" v-model="formData.available_amount" label="Available Amount"
                    placeholder="Enter available amount"></TheField>
                <div v-if="errorMessage.available_amount" class="text-sm text-red-500">{{ errorMessage.available_amount }}
                </div>

                <!-- Max Pets -->
                <TheField id="max_pets" v-model="formData.max_pets" label="Max Pets" placeholder="Enter max pets">
                </TheField>
                <div v-if="errorMessage.max_pets" class="text-sm text-red-500">{{ errorMessage.max_pets }}</div>

                <!-- Start Letter -->
                <TheField id="start_letter" v-model="formData.start_letter" label="Start Letter"
                    placeholder="Enter start letter"></TheField>
                <div v-if="errorMessage.start_letter" class="text-sm text-red-500">{{ errorMessage.start_letter }}</div>

                <!-- Multiple Image Upload -->
                <input type="file" @change="onImageChange" name="images" id="images" multiple />

                <!-- Display any status or error message -->
                <div v-if="statusMessage" class="text-sm text-red-500">{{ statusMessage }}</div>

                <!-- Submit Button -->
                <TheButtonAuth colort="bg-primary-green-100" colorb="bg-primary-green-200">Create Room Type</TheButtonAuth>
            </form>
        </div>
    </section>
</template>
...
const onSubmit = async () => {
    ...
    if (formData.available_amount <= 0) {
        errorMessage.value.available_amount = "Available amount must be greater than 0.";
    }
    ...
    formDataForImages.append("available_amount", formData.available_amount.toString());
