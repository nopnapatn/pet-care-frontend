<template>
  <div class="fix flex-col h-[46rem] justify-center px-48 py-10 gap-y-6">
    <div class="flex w-full justify-center">
      <ol class="flex justify-around w-full">
        ...
        >
          ...
          >
            <svg
              class="w-4 h-4 text-yellow-500 lg:w-5 lg:h-5 dark:text-gray-100"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 20 16"
    ...
    <div class="mx-48 mt-10 border-b">
      <div class="relative overflow-x-auto max-h-64">
        <table
          class="w-full text-sm text-left text-gray-500 dark:text-gray-400"
        >
          <thead class="text-xs text-gray-900 uppercase sticky top-0 bg-white">
            <tr>
    ...
    <div class="flex mt-8 mx-48 mr-68 justify-between">
      <h1>Pet Type</h1>
      <h1>
        {{ typeof petType === "string" ? petType.toLowerCase() : petType }}
      </h1>
    </div>
    <div class="flex mt-8 mx-48 mr-68 justify-between">
      <h1>Booked Date</h1>
    ...
    <div class="flex justify-center mt-12">
      <a @click="confirm()" class="btn btn-primary text-white"> Confirm </a>
    </div>
  </div>
</template>
...
const alacarteIDs = (route.query.alacarteIDs as string)
  .split(",")
  .map((id) => parseInt(id, 10));

const fetchedServiceItems = await useMyFetch<any>("service-items", {
  method: "GET",
});

const serviceItems = fetchedServiceItems.data.value;

const packageItem = serviceItems.find(
  (item: any) => item.id === Number(packageID)
);

const alacarteItems = serviceItems.filter((item: any) =>
  alacarteIDs.includes(item.id)
);
