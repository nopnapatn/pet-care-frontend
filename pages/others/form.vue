      ...
      <ol class="flex justify-around w-full">
        <li
          ...
          <span
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-ui-radios"
              viewBox="0 0 16 16">
              <path
    ...
    <div class="grid grid-cols-3">
      ...
      <div class="flex flex-col px-10 gap-y-4">
        <div class="flex flex-col gap-y-2">
          ...
          <div class="flex gap-4">
            <input type="radio" name="radio-1" value="DOG" v-model="selectedPet" class="text-secondary focus:ring-primary"
              checked />
            <label for="radio1">Dog</label>
          </div>
          <div class="flex gap-4">
            <input type="radio" name="radio-1" value="CAT" v-model="selectedPet"
        ...
        <div class="flex flex-col gap-y-2">
          ...
          <div class="flex gap-4">
            <input type="radio" name="radio-2" value="Spa Bath Package" v-model="selectedPackage"
              class="text-secondary focus:ring-primary" checked />
            <label for="radio2">Spa Bath Package</label>
          </div>
          <div class="flex gap-4">
            <input type="radio" name="radio-2" value="All-Inclusive Groom Package" v-model="selectedPackage"
              class="text-secondary focus:ring-primary" />
            <label for="radio3">All-Inclusive Groom Package</label>
          </div>
          <div class="flex gap-4">
            <input type="radio" name="radio-2" value="None" v-model="selectedPackage"
              class="text-secondary focus:ring-primary" />
      ...
      <div class="flex flex-col px-10">
        <div class="flex flex-col gap-y-4 items-start mb-4">
          <div class="flex flex-col gap-y-2 w-full">
            ...
            <div v-if="selectedPackage === 'Spa Bath Package'">
              ...
              <div v-for="item in spaItems" :key="item.id">
                ...
                <div class="flex gap-4">
                  <input type="radio" name="radio-3" :value="item" v-model="selectedType"
                    class="text-secondary focus:ring-primary" />
            ...
            <div v-else-if="selectedPackage === 'All-Inclusive Groom Package'">
              ...
              <div v-for="item in groomItems" :key="item.id">
                <!-- Render item details here using item -->
                <div class="flex gap-4">
                  <input type="radio" name="radio-3" :valur="item" v-model="selectedType"
                    class="text-secondary focus:ring-primary" />
                  <div class="flex justify-between w-full">
                    <label for="radio3">{{ item.option }}</label>
                    <h1 class="text-md">${{ item.price }}</h1>
          ...
          <div v-for="item in alacarteItems" :key="item.id">
            <div class="flex justify-between w-full">
              <div class="flex items-center">
                <input :id="`checkbox-${item.id}`" type="checkbox" :value="item" v-model="selectedAlacarte"
                ...
                <label :for="`checkbox-${item.id}`" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{
                  item.option }}</label>
              </div>
              <h1>+ ฿{{ item.price }}</h1>
            </div>
      ...
      <div class="bg-primary/20 mx-6 px-8 h-fit pb-8 rounded-lg">
        ...
        <a @click="navigateToServiceReport()" 
        class="btn btn-primary text-white bg-secondary w-full">
          Proceed
        </a>
        <!-- <NuxtLink to="/others/report" class="btn btn-primary text-white bg-secondary w-full">
          Proceed
        </NuxtLink> -->
      </div>
    </div>
...
const groomItems: ServiceItem[] = serviceItems.filter((item) => item.service_name === "All-Inclusive Groom Package");
...
async function navigateToServiceReport() {
  ...
  await navigateTo(
    { path: '/others/report', query: queryParams }
  );
}
